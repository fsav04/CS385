/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Frank Savattere and Aidan Ryan
 * Version     : 1.0
 * Date        : 11/23/2021
...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        Node<K, V>* Nod = new Node<K, V>(key, value);
        Nod -> parent = y;
        if(y == NULL){
            root_ = Nod;
        }
...
delete_tree(n -> right);
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
        ...
        if(node == NULL){
            ...
            sum_levels(node -> left, level + 1) + level;
        }
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
        ...
        if(node == NULL){
            ...
            return sum_null_levels(node -> right, level + 1) + 
            sum_null_levels(node -> left, level + 1);
        }
}
};
